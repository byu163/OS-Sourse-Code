...
Framebuffer* InitializeGOP(){
	...
	if (EFI_ERROR(status)){
		Print(L"Unable to locate GOP (Graphics Output Protocol)\n\r");
		return NULL;
	}
	else
	{
...
EFI_STATUS efi_main (EFI_HANDLE ImageHandle, EFI_SYSTEM_TABLE *SystemTable) {
	...
	{
		Print(L"Kernel header successfully vefified\n\r");
	}
	...
	PSF1_FONT* newFont = LoadPSF1_Font(NULL, L"zap-light16.psf", ImageHandle, SystemTable);
	if (newFont == NULL){
		Print(L"Font is not valid. Error Code 10: newFont == NULL. More info: Error Code: 10 Must have the name zap-light16.psf. Error Code: 10 File Not Found. Error Code: 10 Not Included in Makefile.\n\r");
	}
	else
	{
		Print(L"Font valid / found \n\r charsize = %d \n\r", newFont->PSF1_Header->charsize);
	}
